...
func (b *Broker) getSession(cli *client, req *packets.ConnectPacket, resp *packets.ConnackPacket) error {
	...
	// session and start a new one. b session lasts as long as the network
	// connection. State data associated with b session must not be reused in any
	// subsequent session.
	...
	// If found, return it.
	if !req.CleanSession {
		if cli.session, err = b.sessionManager.Get(cid); err == nil {
	...
	if cli.session == nil {
		if cli.session, err = b.sessionManager.New(cid); err != nil {
			return err
		}

		resp.SessionPresent = false

		if err := cli.session.Initialize(req); err != nil {
			return err
		}
	}
